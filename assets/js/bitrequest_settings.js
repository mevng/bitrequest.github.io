...
function submitaccount() {
	$(document).on("click touch", "#accountformbox input.submit", function(e) {
		e.preventDefault();
		var thisvalue = $(this).prev("input").val();
	...
	$(document).on("click touch", "#currencysettings", function() {
		...
		$.each(symbolstringarray, function(key, value) {
			if (key == "BTC") { // remove from list
				...
				symbollist += "<span data-id='1'>" + key + " | " + value + "</span>";
			}
		});
		var content = "\
...
function backupdatabase() {
	...
	var gd_active = (GD_auth_class() === true),
		...
		gdtrigger = (GoogleAuth) ? "<div id='gdtrigger' class='clearfix'><h3 class='icon-googledrive'>Backup with Google Drive" + switchpanel(gd_active, " custom") + "</h3></div>" : "",
		...
		<div id='backupactions'>\
			<a href='data:application/octet-stream;charset=utf-16le;base64," + jsonencode + "' download='" + filename + "' id='triggerdownload' class='customtrigger icon-download' data-date='" + new Date($.now()).toLocaleString(language).replace(/\s+/g, '_').replace(/\:/g, '_') + "' data-lastbackup='" + filename + "' download>DOWNLOAD BACKUP</a>\
...
function complilebackup() {
	...
	for (var key in localStorage) {
		...
		if (value !== null && key != "bitrequest_symbols" && key != "bitrequest_phpsupport" && key != "bitrequest_changes" && key != "bitrequest_erc20tokens" && key != "bitrequest_editurl" && key != "bitrequest_backupfile_id" && key != "bitrequest_k") {
...
function submitbackup() {
	$(document).on("click touch", "#triggerdownload", function() {
		var thisnode = $(this),
...
function trigger_restore() {
	var restorenode = $("#restore"),
		...
		listappdata = (GoogleAuth) ? "<div id='listappdata'><h3 class='icon-googledrive'>Restore from Google drive:" + switchpanel(gd_active, " custom") + "</h3></div>" : "",
			...
			if (filetype == 'application/json') {
				...
				reader.onload = function(e) {
					var filesrc = e.target.result,
						base64decode = atob(filesrc.substr(filesrc.indexOf(",") + 1)),
						jsonobject = JSON.parse(base64decode);
					$("#fileupload").data({
						"jsonobject": jsonobject,
						"fileused": filename
					});
					$("#restoreformbox input.submit").addClass("restore_active");
				};
...
function submitrestore() {
	$(document).on("click touch", "#restoreformbox input.submit", function(e) {
		e.preventDefault();
		if ($(this).hasClass("restore_active")) {
			var fileupload = $("#fileupload"),
				jsonobject = fileupload.data("jsonobject"),
				filename = fileupload.data("fileused"),
				result = confirm("Restore " + filename + "?");
			if (result === true) {
...
function cachecontrol() {
	$(document).on("click touch", "#cachecontrol", function() {
		var playstorelink = "https://play.google.com/store/apps/details?id=io.bitrequest.app",
			...
			(is_ios_app === true) ? appstorelink : playstorelink,
			lfu = (offline === true) ? "" :
			(supportsTouch === true) ? "<br/><a href='" + appstore_url + "' class='button'>Look for updates</a>" : "",
			emptycache_string = (hostlocation != "local" && offline !== true) ? "<br/><br/><div class='button' id='clearcache'>Clear cache</div>" : "",
			content = "\
				<div class='formbox' id='cacheformbox'>\
					<h2 class='icon-database'>Cache control</h2>\
					<div class='popnotify'></div>\
					<div class='popform'>" + lfu + emptycache_string + "\
...
function clearcache() {
	$(document).on("click touch", "#clearcache", function() {
		...
		if (result === true) {
			...
			localStorage.removeItem("bitrequest_k");
		}
	})
}
	...
	$(document).on("click touch", "#themesettings", function() {
		...
		$.getJSON(approot + "api/custom/?themes", function(data) {
			...
			$.each(data, function(key, value) {
				options.append("<span>" + value + "</span>");
			});
		});
	})
}
	...
	$(document).on("click touch", "#usformbox input.submit", function(e) {
		...
		if (thisvalue == currentapi && firebase_checkchange == firebase_apival && bitly_checkchange == bitly_apival && !toggle_urlshortener.hasClass("us_changed")) { // check for changes
			...
			if (thisvalue != currentapi || toggle_urlshortener.hasClass("us_changed")) {
				...
				us_settings.data({"selected": us_title, "us_active": us_state}).find("p").html(us_title);
			}
			if (us_active === true) {
...
function editccapi() {
	$(document).on("click touch", "#cmcapisettings", function() {
		var cc_apisettings = $("#cmcapisettings").data(),
			...
			options = "<span>" + apilists.crypto_price_apis.join("</span><span>") + "</span>",
			content = "\
...
function editfiatxrapi() {
	$(document).on("click touch", "#fiatapisettings", function() {
		var thisdata = $(this).data(),
			...
			options = "<span>" + apilists.fiat_price_apis.join("</span><span>") + "</span>",
...
function json_check_apikey(keylength, thisref, payload, apikeyval, lastinput) {
	if (apikeyval.length > keylength) {
		if (thisref == "infura") {
			...
			if (web3) {
				if (web3.currentProvider.host == infura_testurl) {
				}
				else {
			...
			var api_data = get_api_data(thisref),
				...
				proxy = (thisref == "coinmarketcap") ? false : false,
				params = {
					"method": method,
					"cache": true
				},
...
function showsat() {
	$(document).on("click touch", ".cc_settinglist li[data-id='showsatoshis'] .switchpanel", function() {
		...
		body.addClass("showsatedited").data("currency", thiscurrency);
	})
}
	...
	$(document).on("click touch", ".cc_settinglist li[data-id='blockexplorers']", function() {
		...
		if (options === undefined) {
			...
			$.each(options, function(key, value) {
				optionlist.append("<span>" + value + "</span>");
			});
		}
	})
}
	...
	if (s_id == "apis") {
		...
		if (thiscurrency == "bitcoin") {
			...
			var ping_event = JSON.stringify({
			  	...
			  	params: ["logs",{address:"0x56ba2Ee7890461f463F7be02aAC3099f6d5811A8",topics:[]}]
		    });
		}
...
function rpc_option_li(optionlist, live, key, value, selected, checked) {
	var liveclass = (live === true) ? " live" : " offline",
		...
		option = $("<div class='optionwrap" + liveclass + selected_class + default_class + "' style='display:none'><span" + datakey + " data-value='" + value.url + "'>" + node_name + "</span><div class='opt_icon_box'><div class='opt_icon c_stat icon-" + icon + "'></div><div class='opt_icon icon-bin'></div></div>");
...
function test_rpcnode() {
	$(document).on("click touch", "#settingsbox .selectbox .options > div", function() {
		...
		if (thisoption.hasClass("offline")) {
			...
			//settingsbox.find(".options").removeClass("showoptions");
		}
		return false;
	})
}
...
function test_rpc(rpc_input_box, rpc_data, currency) {
	if (s_id == "apis") {
		if (currency == "ethereum") {
			if (web3) {
				if (web3.currentProvider.host == rpc_data.url) {
				}
				else {
